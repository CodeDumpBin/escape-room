...
export default class CustomGame extends Phaser.Scene {
  ...
  create() {
    ...
    for (let i = 0; i < 28; i++) {
      ...
      let _p = this.physics.add.sprite(x * 100, y * 100, 'usa', i)
      // _p.alpha = 0;
      ...
      _p.on('dragend', function (drag) {
        // console.log("..drag stop", drag);
        for (let j = 0; j < this.pieces.length; j++) {
          // console.log();
          if (this.pieces[j].position !== i && Phaser.Geom !== undefined) {
            // console.log(this.pieces[j].position, i)
            let _intersect = Phaser.Geom.Intersects.GetRectangleIntersection(this.pieces[j].piece.getBounds(), _p.getBounds());
            console.log(_intersect);
            // console.log(Phaser.Geom.Intersects.GetRectangleIntersection(this.pieces[j].piece.getBounds(), _p.getBounds()))
            // Phaser.Geom.Intersects(this.pieces[i].piece.getBounds(), _p.getBounds())
            // console.log("______________________", this.pieces[i].piece.getBounds(), _p.getBounds());
            // console.log(Phaser.Rectangle.intersects(this.pieces[i].getBounds(), _p.getBounds()));
          }
          // console.log(Phaser.Rectangle.intersects(this.pieces[i].getBounds(), _p.getBounds()))
